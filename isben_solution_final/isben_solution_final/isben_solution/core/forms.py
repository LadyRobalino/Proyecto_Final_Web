...
INPUT_CSS = 'w-full rounded-xl border-2 border-slate-400 bg-white px-4 py-2.5 text-sm text-slate-800 shadow-sm transition focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 focus:outline-none placeholder:text-slate-400'
CHECK_CSS = 'h-4 w-4 rounded border-2 border-slate-400 text-indigo-600 focus:ring-indigo-500'

def aplicar_estilos_tailwind(form_instance):
    for field_name, field in form_instance.fields.items():
        widget_name = field.widget.__class__.__name__
        css = CHECK_CSS if widget_name == 'CheckboxInput' else INPUT_CSS
...
class RegistroForm(UsuarioBaseForm):
    ...
    experiencia = forms.CharField(widget=forms.Textarea(attrs={'rows': 2}), required=False, label="Experiencia laboral/ventas")
...
class PerfilUsuarioForm(forms.ModelForm):
    """Datos personales que cualquier usuario puede editar sobre sí mismo (Mi cuenta)."""
...
class AdministradorPerfilForm(forms.ModelForm):
    """El administrador puede editar su área de trabajo, pero no su propio nivel de acceso."""
    ...
    class Meta:
        model = Administrador
        fields = ['area_trabajo']
        labels = {'area_trabajo': 'Área de trabajo'}
...
class EmpresaCreateForm(UsuarioBaseForm):
    ...
    tiempo_entrega_dias = forms.IntegerField(required=False, initial=2, min_value=0,
    ...
    def guardar(self):
        ...
        return Empresa.objects.create(
            ...
            tiempo_entrega_dias=self.cleaned_data.get('tiempo_entrega_dias') or 2,
        )
...
class EmpresaForm(forms.ModelForm):
    ...
    class Meta:
        model = Empresa
        fields = ['ruc', 'razon_social', 'representante_legal', 'direccion', 'logo_url',
                  'tiempo_entrega_dias', 'estado']
...
class VendedorForm(forms.ModelForm):
    ...
    class Meta:
        model = Vendedor
        fields = ['numero_identidad', 'experiencia', 'estado_aprobacion', 'empresas_aprobadoras']
...
class CompradorForm(forms.ModelForm):
    ...
    class Meta:
        model = Comprador
        fields = ['tipo_negocio', 'ruc', 'ciudad', 'direccion_entrega', 'limite_credito']
...
class DireccionEntregaForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ['etiqueta', 'direccion', 'predeterminada']
        widgets = {'direccion': forms.Textarea(attrs={'rows': 2})}
...
class PromocionForm(forms.ModelForm):
    ...
    class Meta:
        model = Promocion
        fields = ['titulo', 'descripcion', 'nivel_minimo', 'descuento_porcentaje',
...
class ProductoForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'activo': forms.Select(choices=[(True, 'Activo'), (False, 'Inactivo')]),
        }
...
class PlanSuscripcionForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ['nombre', 'precio', 'duracion_meses', 'comision_plataforma', 'beneficios', 'limite_productos', 'tipo', 'activo']
        widgets = {
...
class SuscripcionForm(forms.ModelForm):
    ...
    class Meta:
        model = Suscripcion
        fields = ['empresa', 'plan', 'estado', 'fecha_inicio', 'fecha_fin']
        widgets = {
            'fecha_inicio': forms.DateInput(attrs={'type': 'date'}),
            'fecha_fin': forms.DateInput(attrs={'type': 'date'}),
        }
...
class CursoForm(forms.ModelForm):
    activo = forms.ChoiceField(
        choices=[(True, 'Publicado'), (False, 'Borrador')],
        label="Estado",
        widget=forms.Select(attrs={'class': 'form-select'})
    )
    class Meta:
        model = Curso
        fields = ['titulo', 'descripcion', 'duracion_horas', 'url_contenido', 'imagen', 'activo', 'plan_requerido']
        widgets = {
            'descripcion': forms.Textarea(attrs={'rows': 3}),
        }
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        aplicar_estilos_tailwind(self)
        self.fields['plan_requerido'].queryset = PlanSuscripcion.objects.filter(tipo='vendedor', activo=True)
        self.fields['plan_requerido'].label = "Plan Vendedor Requerido"
        self.fields['plan_requerido'].required = False

    def clean_activo(self):
        return self.cleaned_data['activo'] == 'True' or self.cleaned_data['activo'] is True
...
class NotificacionForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['tipo'].choices = [c for c in self.fields['tipo'].choices if c[0] != 'pago']
        self.fields['titulo'].label = "Asunto"
        aplicar_estilos_tailwind(self)
