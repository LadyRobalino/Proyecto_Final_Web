...
class Suscripcion(models.Model):
    ...
    tipo_plan           = models.CharField(max_length=20, choices=TIPO_PLAN)
    ...
    def get_costo_mensual(self):
        ...
        Si el plan es anual, divide entre 12.
        """
        if self.tipo_plan == 'anual':
            return round(self.precio / 12, 2)
        return self.precio
...
class Pedido(models.Model):
    ...
    estado         = models.CharField(max_length=20, choices=ESTADO_PEDIDO, default='creado')
    ...
    def puede_cancelarse(self):
        ...
        """
        return self.estado in ['creado', 'confirmado']
...
class Curso(models.Model):
    ...
    def __str__(self):
        return "%s - %sh (%s)" % (
            self.titulo,
            self.duracion_horas,
            self.empresa.razon_social if self.empresa else "General"
        )
